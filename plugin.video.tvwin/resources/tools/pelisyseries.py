...
referer = 'http://pacific1469.serverprofi24.eu:32400/library/sections/5/all?X-Plex-Token='
...
id_token = plugintools.get_setting("id_token")
...
def series(params):
   ...
   for entry in temp:
    ...
    server_url2 = ser + server_url2 + plex + id_token 
    plugintools.log("server_url2= "+server_url2)
    mini_url = plugintools.find_single_match(entry, 'thumb="([^"]+)"')
    mini_url = ser + mini_url + plex + id_token 
    plugintools.log("mini_url= "+mini_url)
    server_title = plugintools.find_single_match(entry, 'title="([^"]+)"')
    plot = plugintools.find_single_match(entry, 'summary="([^"]+)"')
    plugintools.add_item(action="url_play13", title = server_title , plot = plot , thumbnail = mini_url , url = server_url2 , fanart = pel , info_labels = None , folder = True, isPlayable = False)
...
def url_play12(params):
   ...
   for entry in temp:
    ...
    play32 = ser2 + play32 + plex + id_token 
    tum = plugintools.find_single_match(entry, 'thumb="([^"]+)"')
    tum = ser + tum + plex + id_token 
...
def url_play13(params):
   ...
   for entry in temp:
    ...
    plot = plugintools.find_single_match(entry, 'summary="([^"]+)"')
    plugintools.set_view(TV_SHOWS)
    plugintools.add_item(action="play2", title = url2 , plot = plot , thumbnail = tum , url = play32 , fanart = tum , info_labels = None , folder = False , isPlayable = 	True)	
